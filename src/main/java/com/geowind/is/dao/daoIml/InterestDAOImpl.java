...
public class InterestDAOImpl extends BaseDaoImpl<Picture> implements
		InterestDAO {
	...
	public List<Picture> queryImageByInterest(String vid) {
		...
		String sql = "SELECT picture.pid,pname,picture.location from interest inner "
				...
				+ "	 join picture on picture.pid = adaptorlabel.pid where interest.vid= ?";

		List<Picture> pictureList = queryForList(sql, vid);
